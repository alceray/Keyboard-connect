...
async function updateAllKeyboards(infoJsonList) {
  const insertStatement = `INSERT OR IGNORE INTO all_keyboards (vid, pid, name, json) VALUES (?, ?, ?, ?)`;
  db.serialize(() => {
    ...
    for (const infoJson of infoJsonList) {
      ...
      stmt.run([vid, pid, keyboardName, infoString], (err) => {
        if (err) {
  ...
  console.log("Updated all keyboards");
}
